...
resource "azuread_application_federated_identity_credential" "default" {
  ...
  description    = "Federated identity credential for KeyVault-Acmebot authentication"
...
resource "azurerm_user_assigned_identity" "default" {
  name                = "uai-acmebot-${random_string.random.result}"
  location            = azurerm_resource_group.default.location
  resource_group_name = azurerm_resource_group.default.name
}
...
module "keyvault_acmebot" {
  ...
  auth_settings = {
    ...
    active_directory = {
      client_id                  = azuread_application.default.client_id
      client_secret              = azurerm_user_assigned_identity.default.client_id
